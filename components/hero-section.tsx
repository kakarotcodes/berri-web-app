...
// Simplified animated text - animates words instead of characters
  ...
  return (
    ...
    >
      <nav className="mx-auto max-w-6xl">
        <div
          className={`rounded-full px-4 sm:px-6 py-3 transition-all duration-300 ${
            isScrolled
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <Link
                href="#plans"
                className="hidden sm:inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 text-sm font-semibold rounded-full bg-foreground text-background hover:opacity-90 transition-opacity"
              >
        ...
        <div
          className={`md:hidden overflow-hidden transition-all duration-300 ${
            menuOpen ? "max-h-48 opacity-100 mt-2" : "max-h-0 opacity-0"
          }`}
        >
          <div
            className={`rounded-2xl px-2 py-2 space-y-1 ${
              isScrolled
                ? "bg-background/80 backdrop-blur-xl border border-white/10 shadow-lg"
          ...
          >
            {menuItems.map((item) => (
              <Link
                ...
                className="block px-4 py-2.5 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-foreground/5 rounded-xl transition-colors"
              >
                {item.name}
              </Link>
            ))}
            <Link
              href="#plans"
              onClick={() => setMenuOpen(false)}
              className="block px-4 py-2.5 text-sm font-semibold bg-foreground text-background hover:opacity-90 rounded-xl transition-opacity text-center"
            >
              Get Started →
            </Link>
          </div>
...
// Marquee text - using CSS animation instead of JS
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 overflow-hidden py-4 bg-foreground/5 backdrop-blur-sm">
      <div className="flex whitespace-nowrap animate-marquee">
        <span className="text-sm font-medium text-muted-foreground flex items-center gap-8 pr-8">
          {items}
        </span>
        <span className="text-sm font-medium text-muted-foreground flex items-center gap-8 pr-8">
          {items}
    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-1/2 -left-1/4 w-[600px] sm:w-[800px] h-[600px] sm:h-[800px] rounded-full bg-purple-500/20 blur-[100px] sm:blur-[120px]" />
          <div className="absolute -bottom-1/2 -right-1/4 w-[400px] sm:w-[600px] h-[400px] sm:h-[600px] rounded-full bg-teal-500/20 blur-[100px] sm:blur-[120px]" />
        </div>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 py-24 sm:py-32 text-center">
          {/* Badge */}
          <motion.div
          ...
          >
            <Link
              href="#plans"
              className="group inline-flex items-center gap-3 px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg font-semibold rounded-full bg-foreground text-background hover:opacity-90 transition-opacity w-full sm:w-auto justify-center"
            >
